...
; arguments. One way to define such procedures is to use
; define with dotted-tail notation. In a procedure
; definition, a parameter list that has a dot before the
; last parameter name indicates that, when the procedure
; is called, the initial parameters (if any) will have as
; values the initial arguments, as usual, but the final
; parameter's value will be a list of any remaining
...
(define (same-parity x . ys)
  (define (iter acc tail)
    (if (null? tail)
        acc
        (iter
              (if (eq? (even? x) (even? (car tail)))
                  (append acc (list (car tail)))
                  acc)
              (cdr tail))))
  (iter (list x) ys))
